...
We use Prettier for formatting. Rules are stored in `.prettierrc` (already committed). Current key settings: `printWidth=120`, `tabWidth=2`, `semi=true`, `singleQuote=true`, `trailingComma=es5`, `arrowParens=avoid`, `endOfLine=lf`.
...
Ignore heavy/generated paths by adding `.prettierignore`:
...
- Prettier runs in PR checks via `.github/workflows/pr-checks.yml` with `npx prettier --check .`.
- Keep `.prettierrc` and `.prettierignore` up to date to control CI behavior.

Optional (local convenience):

- Pin Prettier and add scripts:
  ```bash
  npm i -D prettier
  ```
  ```json
...
_Note: The Terraform Cloud outputs can be found in the workspace's "Outputs" tab after a successful apply._
