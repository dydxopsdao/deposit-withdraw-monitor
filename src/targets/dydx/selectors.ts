...
export const dydxSelectors = {
  ...
  accountMenuButton: ((p) =>
    p.getByRole('button', { name: new RegExp(`^(?:MetaMask)?\\s*${DYDX_ADDRESS_RE.source}`, 'i') })) as Selector,
  ...
  /* deposit */
  fundsDialog: ((p) => p.getByRole('dialog', { name: /^(deposit|withdraw)$/i }).first()) as Selector,
  ...
  depositFundsButton: ((p) =>
    dydxSelectors.fundsDialog(p).getByRole('button', { name: /^deposit funds$/i })) as Selector,
  ...
  chainPickerDialog: ((p) => p.getByRole('dialog', { name: /select (a )?chain/i })) as Selector,

  // A specific chain row in that dialog, e.g. "Ethereum", "Polygon", …
  chainPickerRow: ((p, chain: string) =>
    dydxSelectors
      .chainPickerDialog(p)
      .getByRole('button', { name: new RegExp(`\\b${esc(chain)}\\b`, 'i') })) as Selector,

  withdrawFundsButton: ((p) => dydxSelectors.fundsDialog(p).getByRole('button', { name: 'Withdraw' })) as Selector,
};
